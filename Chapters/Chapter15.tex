...
\end{remarkenv}
\begin{definitionenv}
    Suppose that $S\subseteq \R^n$ is a bounded region and there is a hyperrectangle $Q = [\bm a, \bm b]$ such that $S \subseteq Q$. Let $f$ be a scalar field defined and bounded on $S$. We define the \textbf{extended scalar field} $\tilde f$ of $f$ on $Q$ by
    $$
    ...
    \end{cases}
    $$
    We say that $f$ is \textbf{integrable} on $S$ if $\tilde f$ is integrable on $Q$. In this case, we define and denote the \textbf{integral} of $f$ on $S$ by
    $$
    \int \cdots \int_S f(x_1, \ldots, x_n) \dd x_1 \cdots \dd x_n := \int \cdots \int_Q \tilde f(x_1, \ldots, x_n) \dd x_1 \cdots \dd x_n.
    $$
\end{definitionenv}
\begin{theoremenv}
...
\end{propositionenv}
\begin{proofenv}
    By Lemma~\ref{lem:scaling for vol of n-ball}, it suffices to show $V_{2m}(1) = \frac{\pi^m}{m!}$ for any positive integer $m$. We prove this by induction on $m$. 
