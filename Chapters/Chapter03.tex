	...
	\end{corollaryenv}
	\begin{theoremenv}
		Let $f$ be nonnegative and integrable on $[a,b]$ and let $\mathcal{S}$ be its ordinate set. We define a new function $g$ denoted by
		\[
		g(x):=kf\left( \frac{x}{k}\right) \qquad \text{with}\ x\in[ka,kb],\, k>0
		\]
		and its ordinate set $k\mathcal{S}$. Then the area of $k\mathcal{S}$ is given by
		\[
		A(k\mathcal{S})=k^2A(\mathcal{S}).
		\]
	\end{theoremenv}
	\begin{remarkenv}
