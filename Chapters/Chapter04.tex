	...
	\end{definitionenv}
	\begin{remarkenv}
		You've seen and you will see infinitely many ``whenever" which all mean the same thing: ``for all ...".
	\end{remarkenv}
	\begin{corollaryenv}
	...
	\end{proofenv}
	\begin{remarkenv}
		When we choose $f$ and $h$ to apply the squeeze theorem, they're usually not unique as long as they work well.
	\end{remarkenv}
	\begin{theoremenv}
	...
	\end{theoremenv}
	\begin{remarkenv}
		The Intermediate Value Theorem is a immediate consequence of Bolzano's Theorem. Note such $c$ above might not be unique.
	\end{remarkenv}
	...
	\end{proofenv}
    \begin{remarkenv}
        You can also expand the above theorem to what is so-called \textbf{Weighted Mean Value Theorem for Integrals} mentioned in Chapter 3. The proof is left as an exercise (since it's quite analogous to the above proof).
    \end{remarkenv}
	\newpage